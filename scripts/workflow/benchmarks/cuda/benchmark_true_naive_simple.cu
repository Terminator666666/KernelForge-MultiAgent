...
// 最终优化版本（简化的 shared memory tiling）
...
float benchmark_softmax_naive(const float* d_input, float* d_output, int batch_size, int seq_len, int iterations) {
...
float benchmark_softmax_opt(const float* d_input, float* d_output, int batch_size, int seq_len, int iterations) {
    cudaEvent_t start, stop;
    cudaEventCreate(&start);
    cudaEventCreate(&stop);
...
// Similar benchmark functions for other kernels...
// (为简洁起见，省略其他kernel的benchmark函数，实际代码中需要补充)
...
int main() {
    ...
    // 1. Softmax 测试
    {
        ...
        float naive_time = benchmark_softmax_naive(d_input, d_output, batch_size, seq_len, ITERATIONS);
        float opt_time = benchmark_softmax_opt(d_input, d_output, batch_size, seq_len, ITERATIONS);
