...
def _prepare_sparse_kv(ckv_cache, kpe_cache, sparse_indices, head_dim_ckv, head_dim_kpe):
    ...
    kk = torch.cat((kc, kp), dim=-1).transpose(-1, -2).contiguous()

    prepared = (invalid_mask, kc, kk)
...
def run(q_nope, q_pe, ckv_cache, kpe_cache, sparse_indices, sm_scale):
    ...
    invalid_mask, Kc, KcatT = _prepare_sparse_kv(
        ckv_cache=ckv_cache,
        kpe_cache=kpe_cache,
    ...
    q = torch.cat((q_nope, q_pe), dim=-1).to(torch.float32)
