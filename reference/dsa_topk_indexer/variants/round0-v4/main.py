...
def _tensor_key(tensor):
    """用 tensor 身份和布局构造缓存键；缓存值保留源 tensor 引用避免指针复用误命中。"""
    return (
        tensor.device.type,
...
def _cache_limit(cache):
    """不同中间结果使用不同缓存上限，避免大 batch 页展开缓存过早淘汰。"""
    if cache is _DEQUANT_CACHE:
        return _DEQUANT_CACHE_LIMIT
    if cache is _Q_CACHE:
        return _Q_CACHE_LIMIT
    if cache is _WEIGHT_CACHE:
        return _WEIGHT_CACHE_LIMIT
    return _BATCH_CACHE_LIMIT
...
def _remember(cache, key, value):
    ...
    while len(cache) > _cache_limit(cache):
...
def weight_column_cached(weights):
    """缓存 weights[:, :, None] 视图，避免每个 batch 重复创建广播视图。"""
    key = _tensor_key(weights)
    ...
    return _remember(_WEIGHT_CACHE, key, (weights, weight_column))[1]
...
def get_batch_inputs_cached(k_all, k_index_cache_fp8, block_table, seq_lens, b_idx, page_size, head_dim):
    """缓存页表索引、展开 K、K 转置和局部 token 到全局 token 的映射。"""
    ...
    value = (k_index_cache_fp8, block_table, seq_lens, page_indices, k, k_t, token_ids, seq_len)
...
def run(q_index_fp8, k_index_cache_fp8, weights, seq_lens, block_table):
    ...
    weight_column = weight_column_cached(weights)
    ...
    for b_idx in range(batch_size):
        _page_indices, _k, k_t, token_ids, seq_len = get_batch_inputs_cached(
            k_all,
            k_index_cache_fp8,
            block_table,
            seq_lens,
        ...
        scores = q[b_idx] @ k_t
        scores.relu_()
        scores.mul_(weight_column[b_idx])
